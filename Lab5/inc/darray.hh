...
class DArray : public Measurable {
	...
	void prepareThings(unsigned int amount, int mode);
	...
	void fillIncreasingOrder(int amount);
	void fillDecreasingOrder(int amount);
	void fillRandomOrder(int amount);

	int choosePivot(int t1, int t2, int option);
	void quickSort(int l, int r, int mode);
};
